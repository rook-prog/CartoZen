# utils/inset_overview.py (finalized: figure-level inset, robust country/continent, dateline-safe)
# Draw a reliable inset overview WITHOUT affecting main axes layout.
# Country/continent extent is now:
# - Case-insensitive attr lookup (CONTINENT)
# - Multiple fallback hit tests (contains → intersects(buffer) → bbox → nearest)
# - Correct bounds ordering for set_extent (min_lon, max_lon, min_lat, max_lat)
# - Dateline-safe: clamps/normalizes longitudes and avoids wrap glitches
...
def _normalize_longitudes(min_lon, max_lon):
    ...
    def wrap(x):
        ...
        while x > 180: x -= 360
        return x
    a = wrap(min_lon); b = wrap(max_lon)
    # If span crosses dateline (e.g., a=170, b=-170), choose the shorter arc.
    span_direct = b - a
    if span_direct < 0:
        span_direct += 360
...
def _country_and_continent_boxes(bounds):
...
def draw_inset_overview(
    ...
    overlay_edge_color="#0000ff",   # overlay outline colour
    land_color="#f0e8d8",
    ocean_color="#cce6ff",
    aoi_fill_alpha=0.0,
    ...
    try:
        if extent_mode == "aoi":
            ...
            cbox, cont_box, _ = _country_and_continent_boxes(bounds)
