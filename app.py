...
# — Safe extent + clipped text + uncropped export
# Requires:— Safe extent + clipped text + uncropped export
...
if view == "Map":
    ...
    if up_file and stn and at and lab:
        ...
        # Stations
        ax.scatter(df["Lon_DD"], df["Lat_DD"], s=m_size**2, c=m_col, marker=shape_map[shape], transform=ccrs.PlateCarree(), zorder=5)

        # Labels (clip to axes so they never bleed outside the page)
        if show_lab:
            for _, r in df.iterrows():
                ax.text(r["Lon_DD"] + dx, r["Lat_DD"] + dy, str(r[lab]), fontsize=label_f,
                        transform=ccrs.PlateCarree(), path_effects=halo, clip_on=True)
        ...
        if leg_on:
            ...
            lt = ax.text(xp, yp, leg_text, transform=ax.transAxes, fontsize=legend_f,
        ...
        # ── Export: fixed page frame (no tight bbox) ────────────────────────
